...
function emitJs(ir) {
  const lines = ["// generated by PANINI JS bootstrap codegen", "export const paniniIR = " + JSON.stringify(ir, null, 2) + ";"];
  for (const fn of ir.functions) {
    const params = (fn.params || []).join(", ");
    lines.push(`export function ${safeIdent(fn.name || "anon")}(${params}) {`);
    lines.push(`  throw new Error("Execute via PANINI runtime, not raw JS emit for ${fn.name}");`);
    lines.push("}");
  }
  return lines.join("\n");
}
